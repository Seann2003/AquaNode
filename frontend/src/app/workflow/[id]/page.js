...
import {
  ...
  upsertWorkflow as lsUpsertWorkflow,
...
const mockWorkflowData = {
  1: {
    ...
    description:
      'Monitor wallet balance and get AI insights on portfolio performance',
    status: 'active',
    created: '2024-01-15',
    ...
    blocks: [
      ...
      },
      {
        ...
        config: {
          prompt:
            'Analyze the portfolio balance and provide insights on performance and recommendations',
          includeContext: true,
          model: 'GPT-4',
        },
        status: 'success',
    ...
    executionHistory: [
      {
        ...
        status: 'success',
        duration: '4.7s',
        result: 'Portfolio up 7.2%',
      },
      {
        timestamp: '2024-01-20 10:25:00',
        status: 'success',
        duration: '4.1s',
        result: 'Portfolio up 6.8%',
      },
      {
        timestamp: '2024-01-20 10:20:00',
        status: 'success',
        duration: '5.2s',
        result: 'Portfolio up 6.5%',
      },
      {
  ...
  const normalizeAI = (aiObj) => {
    ...
    const isNested =
      ...
      (a.response.explanation ||
        a.response.rawResponse ||
        a.response.insights?.length ||
        a.response.recommendations?.length);
  ...
  const handleRunOnce = async () => {
    ...
    try {
      ...
      const durationSeconds =
        ((Date.now() - runStartMs) / 1000).toFixed(1) + 's';
      ...
      const summarizeBalance = (balanceObj, chain) => {
        ...
        if (balanceObj.native) {
          const symbol =
            balanceObj.native.symbol ||
            (chain?.includes('Oasis')
              ? 'ROSE'
              : chain?.includes('Sui')
              ? 'SUI'
              : 'Native');
          return `${
            balanceObj.native.formatted ?? balanceObj.native.balance
          } ${symbol}`;
        }
      ...
      const summaryLines = execution.results.map((r) => {
        if (
          r.blockType === 'walletBalance' ||
          r.result?.type === 'wallet_balance'
        ) {
          const base = r.result?.type ? r.result : r.result?.result;
          const short = summarizeBalance(
            base?.balance,
            base?.chain || r.blockName
          );
          const shortAddr = (base?.address || '').slice(0, 6);
          return `Balance (${base?.chain || 'chain'} ${
            shortAddr ? shortAddr + '...' : ''
          }): ${short}`;
        }
        if (r.blockType === 'balancesByAddress') {
          const base = r.result?.type ? r.result : r.result;
          const arr = Array.isArray(base?.data?.data)
            ? base.data.data
        ...
        if (
          r.blockType === 'walletTransaction' ||
          r.result?.type === 'wallet_transactions'
        ) {
          const base = r.result?.type ? r.result : r.result?.result;
          const txs = Array.isArray(base?.transactions)
            ? base.transactions
            : [];
          ...
          const shortAddr = (base?.address || '').slice(0, 6);
          return `Transactions (${base?.chain || 'chain'} ${
            shortAddr ? shortAddr + '...' : ''
          }): ${count} found, latest ${latestType}`;
        }
        if (r.blockType === 'tokenInfo' || r.result?.type === 'token_info') {
          ...
          if (base?.success === false) {
            return `${r.blockName}: ${
              base.error || 'Failed to fetch token info'
            }`;
          }

          if (base?.source === 'uniswap_v3' && base?.coin) {
            // Coin symbol result from theGraphService
            const price = base.data?.currentPrice
              ? `$${base.data.currentPrice.toFixed(6)}`
              : 'N/A';
            const change = base.data?.priceChange24h
              ? `${base.data.priceChange24h >= 0 ? '+' : ''}${
                  base.data.priceChange24h
                }%`
              : 'N/A';
            return `${r.blockName}: ${base.coin} - ${price} (${change} 24h)`;
          } else if (
            base?.source === 'blockchain_service' &&
            base?.tokenAddress
          ) {
            // Token address result from blockchainService
            const addr = base.tokenAddress.slice(0, 8) + '...';
            return `${r.blockName}: Token ${addr} on ${base.chain}`;
          }

          return `${r.blockName}: token info fetched`;
        }
        if (
          r.blockType === 'aiExplanation' ||
          r.result?.type === 'ai_explanation'
        ) {
          const base = r.result?.type ? r.result : r.result?.result;
      ...
      setWorkflow((prev) => {
        const previousHistory = Array.isArray(prev.executionHistory)
          ? prev.executionHistory
          : [];
        const updatedHistory = [newHistoryEntry, ...previousHistory];
        const updatedTotalRuns = (prev.totalRuns || 0) + 1;
        const successfulRuns = updatedHistory.filter(
          (h) => h.status === 'success'
        ).length;
        const updatedSuccessRate = Number(
          ((successfulRuns / updatedHistory.length) * 100).toFixed(1)
        );
        ...
        const updated = {
          ...
          successRate: isFinite(updatedSuccessRate)
            ? updatedSuccessRate
            : prev.successRate || 0,
        ...
        const userId =
          prev?.ownerId || user?.wallet?.address || user?.id || 'anonymous';
        try {
    ...
    <div className='min-h-screen bg-background pt-16'>
      <div className='max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8'>
          ...
          <div className='flex items-center space-x-4'>
            ...
            <div>
              <h1 className='text-3xl font-bold text-foreground'>
                {editableName || workflow.name}
              </h1>
              <p className='text-foreground/70 mt-1'>
                {editableDescription || workflow.description}
              </p>
            </div>
          </div>
        ...
        <div className='flex space-x-1 mb-8'>
          {['overview', 'analysis', 'blocks', 'history', 'settings'].map(
            (tab) => (
              <button
                key={tab}
              ...
              if (!ai)
                return (
                  <p className='text-sm text-foreground/60'>
                    No AI analysis yet. Run the workflow to generate one.
                  </p>
                );
              return (
      ...
      {block.type === 'balancesByAddress' && block.lastResult && (
        <div className='bg-background/50 rounded-lg p-4 mt-4'>
          ...
          {(() => {
            ...
            if (!arr || arr.length === 0) {
              return (
                <p className='text-sm text-foreground/60'>No balances found.</p>
              );
            }
            const net = networkIdToName(
              res?.metadata?.networkId || block.config?.networkId || ''
            );
            return (
              <div className='space-y-2'>
                <p className='text-sm text-foreground/70'>
                  Network: <span className='text-foreground'>{net}</span>
                </p>
